...
class UpdateProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'variants.*.quantity' => 'required|min:0',
            'variants.*.variant_import_price' => 'required|min:0',
            'variants.*.variant_sale_price' => 'required|min:0',
            'variants.*.variant_listed_price' => 'required|min:0',
            'variants.*.image' => 'nullable|image|max:5048',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            foreach ($variants as $index => $variant) {
                $sizeId = $variant['attribute_size_name'] ?? null;
                $colorId = $variant['name'] ?? null;

                // Kiểm tra xem cặp size và màu đã tồn tại chưa
                if (in_array([$sizeId, $colorId], $sizeColorPairs)) {
                    $validator->errors()->add("variants.$index.attribute_size_name", 'Kết hợp size và màu không được trùng lặp.');
                } else {
